...
public class App {
    ...
    public void genericXmlApplicationContext() throws Exception {
        ...
        resources = classLoader.getResources("com/shareniu/chapter11/applicationContext.xml");
    ...
    public void addInputStreamTest() {
        //定义的文件信息的流读取
        InputStream inputStream = DeploymentBuilderTest.class.getClassLoader().getResourceAsStream("com/shareniu/chapter11/1.bpmn");
        //流程定义的分类
    ...
    public void startProcessInstanceById() throws Exception {
        ...
        variables.put("cc", "test");
        variables.put("deptLeaderApproved", true);
        variables.put("hrApproved", true);
        //variables.put("", new ShareniuDelegateExpression())
        ProcessInstance startProcessInstanceById = runtimeService.startProcessInstanceById("myProcess:4:80004", variables);
        System.out.println(startProcessInstanceById);
        Task task = taskService.createTaskQuery().processInstanceId(startProcessInstanceById.getProcessInstanceId()).active().singleResult();
        System.out.println(task.getId());
        taskService.complete(task.getId());
        task = taskService.createTaskQuery().processInstanceId(startProcessInstanceById.getProcessInstanceId()).active().singleResult();
        System.out.println(task.getId());
        taskService.complete(task.getId());
        //Thread.sleep(1000);
    }
